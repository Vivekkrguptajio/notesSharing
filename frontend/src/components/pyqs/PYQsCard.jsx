...
import { FileQuestion, Download, Star, User, CheckCircle, Calendar, Eye } from "lucide-react";

export default function PYQsCard({ pyq }) {
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 sm:p-5 hover:shadow-lg transition-all duration-300 hover:border-indigo-200">
            ...
            <div className="flex items-start gap-3 mb-3">
                <div className="flex-shrink-0 w-10 h-10 sm:w-12 sm:h-12 bg-indigo-50 rounded-lg flex items-center justify-center">
                    <FileQuestion className="w-5 h-5 sm:w-6 sm:h-6 text-indigo-600" />
                </div>
                <div className="flex-1 min-w-0">
                    <h3 className="text-sm sm:text-base font-semibold text-gray-900 mb-1 line-clamp-2">
                        {pyq.title}
                    </h3>
                    <p className="text-xs sm:text-sm text-gray-600">{pyq.subject}</p>
                </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2 mb-3">
                ...
                <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium ${getExamTypeColor(pyq.examType)}`}>
                    {pyq.examType}
                </span>
            ...
            <div className="flex items-center justify-between text-xs sm:text-sm text-gray-600 mb-4">
                <div className="flex items-center gap-1">
                    ...
                    <span className="truncate max-w-[100px] sm:max-w-none">{pyq.uploadedBy}</span>
                </div>
                <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1">
                        <Star className="w-3 h-3 sm:w-4 sm:h-4 fill-yellow-400 text-yellow-400" />
                        <span className="font-medium">{pyq.rating}</span>
                    </div>
                    <div className="flex items-center gap-1">
